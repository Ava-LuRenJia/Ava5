...
# 定义一个函数来执行K-means聚类并绘制分类结果
def kmeans_classification(K, data):
    # 创建KMeans模型，指定聚类的数量K
    ...
    plt.scatter(data[:, 0], data[:, 1], c=labels, cmap='viridis')  # 根据标签着色
    plt.scatter(centers[:, 0], centers[:, 1], s=200, c='red', marker='X')  # 聚类中心
...
# 执行不同K值的K-means分类并绘制结果
for K in [2, 3, 5]:
    kmeans_classification(K, data)
